...
例外として`src/app/styles/globals.css`だけはフォーマットの対象に含めています。
基盤として今後も触り続けるファイルであり、旧コードとして捨てる対象ではないためです。
そのため`.prettierignore`は`/src/`ではなく`/src/**/*.ts`と`/src/**/*.tsx`の拡張子指定になっています。
`src/`配下の拡張子は`.ts` / `.tsx` / `.css`の3種類しかないので、これでCSSだけが対象に残ります。
なお、除外したディレクトリの配下を`!`で戻すことはgitignoreの仕様上できません。
...
ESLintにはCSSを読むプラグイン (`@eslint/css`) が入っていないため、`.css`を除外から外しても何も起きません。
つまりESLint側は`src/**`のままでよく、**揃える動機がそもそもありません**。

- `.prettierignore`の`/src/**/*.ts`等 = 旧コードは整形しない
- `eslint.config.mjs`の`src/**` = 旧コードの**JS/TS**は検査しない (CSSは元から対象外)

同じ`src/`が両方に書かれているのは見た目の一致で、意味は違います。
