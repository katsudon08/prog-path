...
```
maze（共有用ペイロード） → JSON → 圧縮（deflate 等）→ Base64 → QR
```

- ペイロードに含める: `schemaVersion` / `name` / `size` / `floors` / `tiles`。
- ペイロードに含めない: `id`（インポート時に再採番）/ `folderId`（インポート先で決定）/ `createdAt` / `updatedAt`。
- **サイズ上限**: 圧縮後が QR 容量を超える場合はエラー表示（→ [features.md](./features.md) 3.5）。全階共通サイズ＋密配列＋圧縮で、5×5〜7×7・1〜3 階は 1 QR に収まる想定〔要確認: 上限・圧縮ライブラリ・QR 誤り訂正レベル〕。

### デコード（インポート）

1. QR 文字列 → Base64 デコード → 解凍 → JSON パース。
