...
- **削除できないフォルダには操作メニューを DOM から出さない**（グレーアウトしない）。**児童は無効表示を読まず連打する**。
...
- **チュートリアル迷路は「編集」ではなく「コピーしてつくる」**（複製を未分類に作り、その編集画面へ入る）。Minecraft の Re-Create、Figma の Duplicate と同型。編集導線を出さないことで、**「なぜ削除・編集できないのか」を説明する必要そのものが消える**。
