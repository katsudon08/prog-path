    ...
    return (
        <div className="flex flex-col gap-1.5 h-full">
            {TILE_TYPES.map((tileType) => {
                const icon = getTileIcon(tileType.type, 18)
                return (
                    <button
                        key={tileType.type}
                        onClick={() => onSelectTile(tileType.type)}
                        className={`flex flex-1 w-full items-center gap-2 rounded border-2 px-2 transition-all origin-center hover:scale-[1.05] min-h-0 ${selectedTile === tileType.type
                                ? "border-neon-cyan bg-neon-cyan/10"
                                : "border-neon-blue/30 bg-space-blue/20"
                            }`}
                    >
                        <div className={`h-6 w-6 rounded shrink-0 flex items-center justify-center ${tileType.color}`}>
                            {icon}
                        </div>
