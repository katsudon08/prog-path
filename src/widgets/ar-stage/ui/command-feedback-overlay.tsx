...
export const CommandFeedbackOverlay = ({
  ...
  return (
    <div
      aria-live="polite"
      className={cn(
        "pointer-events-none absolute inset-x-0 top-6 flex justify-center px-4",
        className,
      )}
    >
      <div
        className={cn(
          "inline-flex max-w-full items-center gap-3 rounded-card border border-border bg-background/90 px-5 py-3 text-lg font-bold text-foreground shadow-lg transition-opacity duration-300",
          visible ? "opacity-100" : "opacity-0",
        )}
      >
        {message !== null && (
          <>
            <ScanLine aria-hidden="true" className="size-6 shrink-0 text-primary" />
